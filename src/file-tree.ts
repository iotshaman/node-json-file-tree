...
export class FileTreeGenerator {
    ...
    getTreeData(start) {
        ...
        let files: any;
        let promises = [];
        let path_predicate: string = this.ensurePathFormat(start);
        let start_path: string = this.ensurePathFormat(start);
        if (this.config.windows) {
            path_predicate = this.config.normalize(path_predicate.substring(2));
        }

        promises.push(this.getAllDirectories(start_path)
            .then((rslt) => { return this.mapRelativeNames(rslt, path_predicate, true); })
            .then((rslt) => { folders = rslt; return; })
            .catch((err) => { return Promise.reject(err); })
        );
        promises.push(this.getAllFiles(start_path)
            .then((rslt) => { return this.mapRelativeNames(rslt, path_predicate); })
            .then((rslt) => { files = rslt; return; })
            .catch((err) => { return Promise.reject(err); })
        );

        return Promise.all(promises).then(() => {
            var dir = this.generateDirectory(folders);
            return this.buildFolderStructure(files, dir);
        });
    }
            ...
            this.fs.subdirs(start, (err, subdirs) => {
                ...
                Promise.resolve(subdirs)
                    .then((rslt: Array<string>) => {
                        ...
                        return this.normalizePaths(rslt, this.config.normalize);
                    }).then((rslt: Array<any>) => {
                        if (!this.config.windows) return rslt;
                        return this.convertWindowsPaths(rslt);
                    }).then((rslt: Array<string>) => {
                        return res(rslt);  
                    });
            });
